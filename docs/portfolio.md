...
react to next
- 프리페치 동작 비교? 전환 과정
처럼 서버 컴포넌트가 추가되면서 고려해야되는 점 파악할 것
 -->
<!-- 서류·면접에서 **왜 이렇게 만들었는지**를 설명하기 위한 문서입니다.
[README.md](../README.md)는 짧은 소개이고, 여기는 **이슈 → 어떻게 풀었는지 → 결과** 순으로 적었습니다.
**기술·라이브러리 선택 이유( cookie, RQ, proxy 등 )** 는 [`tech-decisions.md`](./tech-decisions.md) 에 모아 두었습니다. -->
...
| **Next 전환** | App Router — **검색·선택·퀴즈는 client(RQ·Zustand)**, RSC·Edge는 **proxy 가드·Server Action·홈 리그 fetch** 등 요청 경계만 ([아키텍처](#next-architecture) 참고)        |
...
<a id="next-architecture"></a>

### Next.js — client 중심, RSC는 얇게

검색·리그/팀 선택·퀴즈 진행처럼 **사용자 입력과 상호작용이 중심**인 서비스라, 팀·선수 조회와 UI 대부분은 **React Query·Zustand 기반 클라이언트**에 두었습니다. Next App Router의 RSC는 홈 리그 목록 fetch, cookie·가드, Server Action처럼 **요청 경계에서 필요한 부분만** 쓰고, submission 본문처럼 prefetch·persist가 많은 화면은 **의도적으로 client 중심**으로 유지했습니다.
...
- submission **`generateMetadata`** — cookie `leagueId` → 탭 title에 리그명 ✅
- `@/widget` barrel import 정리 — RSC 페이지 직접 import
