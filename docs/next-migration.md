...
```text
selectLeagueAction
  → fetchLeagueList로 leagueId 검증
  → 실패: { ok: false, reason } → NotificationView
  → 성공: cookie league-id → redirect /submission
...
app/layout.tsx
  → <FlashToastView />  (redirect flash 전용)
  → <NotificationView />  (in-app: 리그 목록·선택 실패 등)
```

- `league.store.ts` · `ProtectedRoute` · `Providers` league rehydrate **제거**
- `FlashToastView` + `FlashToastUI` — flash cookie read/delete + enter/exit animation
- `NotificationView` + `showNotificationReason` — same-page 알림 (Zustand store)
...
app/layout.tsx
  └─ FlashToastView (client) — flash cookie read → FlashToastUI (redirect 안내 전용)
  └─ NotificationView (client) — in-app 알림 (리그 목록·선택 실패)
```
...
[리그 선택]
  Server Action (selectLeagueAction)
    → fetchLeagueList로 leagueId 검증
    → 실패: { ok: false, reason: LEAGUE_SELECT_UNAVAILABLE }
    → 성공: cookie에 leagueId 저장 → redirect('/submission')

[useLeagueSelectModal] (client)
  await selectLeagueAction
    → !ok: closeModal + showNotificationReason(reason)
    → redirect throw: unstable_rethrow (Next 내부 처리)

[proxy.ts]  ← 프로젝트 루트 (Next 16: middleware 대신 proxy)
  /submission 접근 시 cookie leagueId **존재** 검증
...
[FlashToastView] (layout, client)
  mount → toast-message cookie read → FLASH_TOAST_MESSAGES → delete → 표시

[NotificationView] (layout, client)
  showNotificationReason → Zustand store → 표시

[client island]
  모달·퀴즈·검색·hover prefetch — leagueId는 page prop
```

**멀티 리그 확장:** RTDB/API 리그 목록이 UI·Action 검증의 단일 소스입니다. `proxy.ts` allowlist 연동 시 Edge에서도 invalid cookie를 차단할 수 있습니다. URL 구조 변경(`/league/[id]/submission`)은 필수가 아닙니다.
...
> **왜 cookie인가 (요약):** `proxy.ts`는 **페이지 렌더 전** Edge에서 동작합니다. React Query·Zustand·sessionStorage는 이 시점에 접근할 수 없고, HTTP cookie만 요청 헤더로 전달됩니다. 상세·면접용 정리는 [`tech-decisions.md`](./tech-decisions.md) §1·§2.

| 파일                            | 역할                                                        |
| ------------------------------- | ----------------------------------------------------------- |
| `fetchLeagueList` + `leagueDto` | 리그 목록 단일 소스 (UI·Action 검증)                        |
| `actions/selectLeagueAction.ts` | API 검증 → `{ ok, reason? }` 또는 cookie set + `redirect`   |
| `useLeagueSelectModal.ts`       | Action 호출 · Notification · `unstable_rethrow`             |
| ~~`league.store.ts`~~           | **제거** — cookie + RSC prop                                |

```typescript
// entities/league/actions/selectLeagueAction.ts (요지)
export type SelectLeagueResult =
  | { ok: true }
  | { ok: false; reason: NotificationReason }

export async function selectLeagueAction(leagueId: number): Promise<SelectLeagueResult> {
  const leagueIds = await fetchLeagueList().then(leagueDto).then(l => l.map(x => x.id))
  if (!leagueIds.includes(leagueId)) {
    return { ok: false, reason: NOTIFICATION_REASON.LEAGUE_SELECT_UNAVAILABLE }
  }
  cookieStore.set('league-id', String(leagueId), { ... })
...
- `league-id` 없으면 `ROUTER_PATH.HOME` redirect + `FLASH_TOAST_REASON.NO_LEAGUE` flash cookie
...
- `FlashToastView` / `FlashToastUI` — redirect flash 전용
- `NotificationView` — in-app 알림 (리그 목록·선택 실패)
...
```typescript
// shared/config/flashToast.ts (요지)
export const FLASH_TOAST_COOKIE_NAME = 'toast-message'
export const FLASH_TOAST_REASON = { NO_LEAGUE: 'no-league' } as const
...
```text
app/
  layout.tsx              Server — metadata, FlashToastView, NotificationView, global.css
...
proxy.ts                  leagueId cookie 존재 검증 + flash toast (루트)

src/entities/league/
  actions/selectLeagueAction.ts   API 검증 + cookie + redirect
  model/useLeagueSelectModal.ts   Action 호출 + Notification

src/shared/
  config/flashToast.ts            FLASH_TOAST_COOKIE_NAME · REASON · MESSAGES
  config/notification.ts          NOTIFICATION_REASON · MESSAGES
  ui/flash-toast/FlashToastView.tsx
  ui/notification/NotificationView.tsx
...
| 위치                     | 작업                                       |
| ------------------------ | ------------------------------------------ |
| Firebase RTDB `leagues`  | 리그 메타·`teamIds`·`playerIds` 데이터 추가 |
| `entities/league/assets` | 엠블럼 이미지 (필요 시)                    |
| `proxy.ts`               | API/allowlist 기반 검증 (연동 예정)        |
