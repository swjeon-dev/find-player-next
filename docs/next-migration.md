...
- [x] styled-components Registry + `compiler.styledComponents` (컴포넌트 styled SSR)
...
| ⬜ | `widget/submission/SubmissionCard` | `SubmissionCard.style.ts` |
| ⬜ | `widget/submission/SubmissionLoader` | TSX 인라인 styled |
| ⬜ | `widget/submission/ChangeButton` | TSX 인라인 styled |
| ⬜ | `shared/ui/skeleton/SkeletonBase` | TSX 인라인 styled |

`ThemeProvider` / `StyledComponentsRegistry` / `styled-components` 패키지 제거는 **위 표 전부 ✅** 이후.
...
- `app/layout.tsx` + `src/app/providers/Providers.tsx` (Query, Recoil, Theme, `global.css`)
...
- styled-components FOUC 대응 — `compiler.styledComponents` + `StyledComponentsRegistry` (`src/app/providers/`)
...
styled-components `GlobalStyle` + `styled-reset`을 제거하고, **빌드 타임 global CSS**로 옮겼습니다. 컴포넌트 styled-components → CSS Modules 전환(§6-1)의 1단계입니다. 진행 상태는 **진행 현황** 참고.
...
:root {
  ...
  --color-accent: #f9c74f;
  --color-header-banner: var(--color-accent);
  --color-header-label: #000000;
...
- **앱 토큰**: `:root` CSS 변수 — `shared/config/theme.ts` `breakpoints`와 동기화 (`@media` 조건에는 `var()` 불가 → px 리터럴)
- **CSS Modules**: 색·간격은 `var(--color-*)` 참조, 반응형은 `@media` (스타일만), JS 분기는 `useBreakpoint` 유지
