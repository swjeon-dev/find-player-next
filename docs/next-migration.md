...
**15 / 15 (100%)** — UI 컴포넌트 styled 제거 완료. **다음:** `ThemeProvider` / `StyledComponentsRegistry` / 패키지 제거.
...
- [ ] **styled-components 인프라 제거** — `ThemeProvider`, `StyledComponentsRegistry`, `next.config` `compiler.styledComponents`, `styled-components`·`styled-reset` 패키지, `theme.ts`·`styled.d.ts` (컴포넌트 SC 전환 완료 후)
...
`:root`에 브레이크포인트·색상 토큰을 둡니다. 전체 목록은 `src/app/styles/global.css` 참고.
...
- **토큰**: `@media` 조건에는 `var()` 불가 → px 리터럴 + `--bp-*` 주석·속성값용 동기화
- **CSS Modules**: 색상은 `var(--color-*)`만 사용 (리터럴 없음). 반응형은 `@media`, JS 분기는 `useBreakpoint`
