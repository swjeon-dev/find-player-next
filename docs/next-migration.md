...
| ✅ | `quizState` (`IFirebasePlayer \| null`) | `useQuizStore` (`quiz.store.ts`) | sessionStorage `quiz-player` · 자동 hydrate (§8-7) |
...
| `quizState` | `quizState.ts` | `useQuizStore` | `quiz.store.ts` | `SubmissionCard`, `useQuizGenerator` |
...
| **현재 퀴즈** | `recoil-persist` + `quizState` | Zustand `persist` · `useQuizStore` | `sessionStorage` | `quiz-player` ※ | 탭 세션 | `IFirebasePlayer` 전체 |
...
※ 퀴즈 storage 키가 Recoil `quizPlayer` → Zustand `quiz-player`로 **변경**됨. 배포 후 기존 탭의 Recoil 퀴즈 캐시는 자동 이전되지 않음 (리그는 §8-4 마이그레이션).
...
| | Recoil | Zustand |
| -- | ------ | ------- |
| sessionStorage 키 | `quizPlayer` | `quiz-player` |
| 마이그레이션 | — | **없음** (키 변경으로 이전 세션 퀴즈는 무시) |
...
| `quiz` store 구조 | 낮음 | `quiz: IFirebasePlayer` → `playerId` + React Query 분리 (데이터·UI 상태 분리 리팩터) |
| `app/submission/page.tsx` `force-dynamic` | 낮음 | 클라이언트 가드·persist 의존. 제거 시 동작 재확인 필요 |
| `keepPreviousData` (`useFetchingPlayerData`) | 낮음 | `quiz` store fallback과 UI 조건 겹침. 제거·단순화 가능 |
...
| persist storage 키·포맷 | ✅ league legacy 호환 · quiz 키 `quiz-player` |
...
src/widget/submission/model/quiz.store.ts   useQuizStore         persist · 자동 hydrate
...
| `useQuizStore` | sessionStorage `quiz-player` | 자동 hydrate (클라이언트) | `SubmissionCard`, `useQuizGenerator` |
...
quiz는 **화면 표시용**이며 `useQuizGenerator`가 fetch 후 `setQuiz`로 채웁니다. persist 자동 hydrate가 서버에서 빈 값으로 끝나도:

- `ProtectedRoute`는 league hydration만 기다림
- 퀴즈 없으면 `generateQuiz()` / fetch로 복구
- 새로고침 시 sessionStorage에서 클라이언트 hydrate로 복원
...
| -- | ------- | ------------------- |
| 데이터 | UI 선택 상태 (리그 ID, 현재 퀴즈, 검색어) | API 응답 캐시 (`queryKey[0] === 'persist'`) |
| Storage | sessionStorage (league, quiz) / 없음 (input) | localStorage |
| 복원 | `Providers.rehydrate()` (league) / 자동 (quiz) | `setupQueryPersist()` |

#### 결론

**Recoil → Zustand 전환 작업은 완료**입니다. 남은 항목은 quiz SSR 패턴 통일·store 구조 리팩터 등 **선택적 개선**이며, 배포·동작을 막는 미완료 항목은 없습니다.
